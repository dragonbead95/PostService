...
<div class="main_content">
	...
	<div class="board_search">
		<form action="/PostService/admin/index" method="get">
			<fieldset>
	...
	<form action="/PostService/admin/board/pub" method="post">
	<table class="board_table">
		...
		<tbody class="board_content">
			...
			<c:forEach var="n" items="${list}">
				...
				<tr>
					<td>${n.id}</td>
					<td><a href="/PostService/admin/board/detail?id=${n.id}">${n.title}</a></td>
					<td>${n.writer_id}</td>
	...
	<div class="board_btn">
		...
		<a href="/PostService/admin/board/reg" class="btn">글쓰기</a>
	...
	<div class="board_page">
		...
		<c:if test="${startNum>1}">
			<a href="/PostService/admin/index?p=${startNum-5}" class="btn_page btn_prev">&lt;</a>
		</c:if>

		<!-- 데이터 레코드 수에 따른 pager 출력 -->
		<c:forEach var="i" begin="0" end="4">
			<c:if test="${(startNum+i)<=lastNum}">
				<a href="/PostService/admin/index?p=${startNum+i}"
		...
		<c:if test="${startNum+4<lastNum}">
			<a href="/PostService/admin/index?p=${startNum+5}" class="btn_page">&gt;</a>
		</c:if>
		<c:if test="${startNum+4>=lastNum}">
